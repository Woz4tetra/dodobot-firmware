    ...
    protected:
        ...
        void parse_packet()
        {
            ...
            if (calc_checksum != recv_checksum) {
                // checksum failed
                write("txrx", "dds", read_packet_num, 4, read_packet);  // error 4: checksums don't match
                read_packet_num++;
                return;
            }
            ...
            if (recv_packet_num != read_packet_num) {
                ...
                write("txrx", "dds", read_packet_num, 6, read_packet);  // error 6: packet counts not synchronized
                read_packet_num = recv_packet_num;
            }

            // find category segment
            if (!next_segment()) {
                write("txrx", "dds", read_packet_num, 7, read_packet);  // error 7: failed to find category segment
                read_packet_num++;
                return;
            }
